...
public class EditorPO extends JFrame {
	...
	private void autoSaveFile() throws RemoteException, InterruptedException {
		...
		if (selectedRow != -1) {
			...
			// Requirement: Auto-Save only triggers if word count > 500
			if (calculateWordCount(content) <= 500) {
				logger.info(
						"Auto-save skipped: Word count is " + calculateWordCount(content) + " (minimum 500 required)");
				return;
			}
